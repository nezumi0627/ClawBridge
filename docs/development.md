...
- `web/`: Next.js ベースの Web フロントエンド。

---

## 🎨 Web UI の開発

ClawBridge のダッシュボードは `web/` ディレクトリにある独立した Next.js プロジェクトです。

### 開発用サーバの起動
UI のみを開発する場合は、以下のコマンドでホットリロード有効な開発サーバが起動します。
```bash
npm run ui:dev
```
※ API 通信を行うには、別途 `node server.js` でバックエンドが起動している必要があります。

### ビルドと反映
メインの ClawBridge サーバが配信する `public/` ディレクトリへ UI の変更を反映するには、以下のビルドコマンドを実行します。
```bash
npm run ui:build
```
このコマンドは内部的に、`web/` での静的エクスポートを実行し、その成果物をルートの `public/` へコピーします。
